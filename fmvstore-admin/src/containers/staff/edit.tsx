...
const EditStaffContainer = ({ id }: { id: string }) => {
  ...
  useFetch(() => UsersService.usersControllerGetStaffDetail({ id: +id }), {
    onSuccess: (data) => {
  ...
  const { run: updateStaff } = useFetch(
    UsersService.usersControllerUpdateStaff,
    {
      manual: true,
      onSuccess: () => {
        toast({
